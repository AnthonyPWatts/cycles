...
`GET /games` is an authenticated account projection. It resolves the current Player independently of Game membership, reads at most the first 100 enrolments through `IGameCatalogueQuery`, and returns a redacted `GamesHomeSnapshot`; it does not load any full Cycle state. The response supplies one server-ordered `games` list, a plural `tutorials` collection, and `hasMore` when further enrolments exist beyond the bounded page.

Each item carries its Game identity and display name, kind and lifecycle, enrolment status, current Cycle summary when one exists, the contextual action (`continue`, `enterLobby`, `observe`, or `review`), and an optional `commandDeadline`. Current Training Games are ordered first, followed by enrolled Games with a known Player command deadline in ascending order; overdue deadlines therefore appear before future ones. Untimed Games use deterministic lifecycle and name fallbacks, with withdrawn enrolments last. `commandDeadline` is present only while an enrolled active Game has an active Cycle whose command stage is open and `nextTickAt` is known. The browser renders this order and deadline meaning but does not reproduce the ranking policy.

An authenticated Player with no enrolments receives a successful empty snapshot. The account shell explains that no Games are available without manufacturing a placeholder Game or calling the legacy bootstrap. When `TrainingGames` exposure includes the Player and no current Twin Reaches attempt exists, the response also carries an entry in `tutorials` with the stable tutorial key, display name, and estimated Core duration. The collection is plural so further curated tutorials can use the same top-of-list contract. When exposure is off, the Player is not allow-listed, or a current attempt already exists, that offer is absent. Existing Training remains visible as a normal Game membership even when creation exposure is later disabled.
