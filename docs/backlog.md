...
The Day One guide remains implemented for Standard games. MG-01 through MG-09 now provide the Game/configuration/enrolment/lifecycle foundation; exhaustive same-scope SQL ownership; Player-only identity and focused stores; explicit selected-Game routes and authority; antiforgery; persisted scheduled/self-paced execution; immutable Standard/Twin Reaches profiles; the Games home; allow-listed private Twin Reaches provisioning; and the server-derived four-resolution Core journey. A Training attempt is an ordinary second Game, uses normal commands and authoritative resolution, persists journey status, presentation acknowledgements, account-level completion/skip evidence, request identity and supersession rather than mechanical progress, and remains excluded from Worker due selection. The integrated MG-01–09 technical gates have passed; the next executable work is MG-10's complete account shell and responsive, accessible journey states, followed by the five-novice Increment 2 pilot gate.

Training provisioning is enabled only through the typed `TrainingGames` pilot audience; disabling exposure prevents new prompts and attempts without removing committed Games. The trusted playground deployment allow-lists Tony for the first pilot. Training now resolves through its own Player-authorised, active-run-gated command; the Development-only manual advance control is hidden for Training. Worker health, leadership, graceful shutdown, monitoring and any later several-Cycle batch policy remain under [#132](https://github.com/AnthonyPWatts/cycles/issues/132). MG-00's baseline and Increment 2 novice evidence remain open.
