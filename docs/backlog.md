...
The current Day One guide and single-Game runtime remain implemented behaviour. The first technical prerequisites are now in place: mandatory non-zero SQL evidence; fail-closed legacy Cycle selection; a frozen online whole-state call-site allowance; additive Game/configuration/enrolment/lifecycle schema; deterministic restartable legacy backfill; v6 transfer with v1-v5 adaptation; legacy dual-write persistence; non-null Cycle/Game ownership; the exhaustive currently representable same-scope relationship matrix; and normalised battle-fleet membership. This completes MG-01 and MG-02's storage/compatibility substance, while MG-00's baseline player evidence remains open. Before code creates a second durable Game, the programme still requires focused player/Game/Cycle stores, every player route migrated or deliberately pinned, explicit Worker due selection, resource authorisation, and antiforgery. The current Worker policy remains under #132 until those bounded slices land.
