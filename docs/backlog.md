...
The current Day One guide remains implemented player behaviour. MG-01 through MG-08 now provide the Game/configuration/enrolment/lifecycle foundation; exhaustive same-scope SQL ownership; Player-only identity and focused stores; explicit selected-Game routes and authority; antiforgery; persisted scheduled/self-paced execution; immutable Standard/Twin Reaches profiles; the Games home; and allow-listed, idempotent private Twin Reaches provisioning. A Training attempt becomes an ordinary second Game, uses the same command and authoritative resolution paths, and remains excluded from Worker due selection. The integrated MG-01–08 gates have passed; the next executable work is MG-09's server-derived Core tutorial journey and recovery/pause/skip/fresh-attempt semantics.

Training provisioning is enabled only through the typed `TrainingGames` pilot audience; disabling exposure prevents new prompts and attempts without removing committed Games. The trusted playground deployment allow-lists Tony for the first pilot. A Training Game currently reuses the existing Development-only manual advance control; a production-safe tutorial resolve command and the four-resolution journey belong to MG-09. Worker health, leadership, graceful shutdown, monitoring and any later several-Cycle batch policy remain under [#132](https://github.com/AnthonyPWatts/cycles/issues/132). MG-00's baseline player evidence also remains open.

The dependency-labelled MG identifiers are planning labels, not live issue numbers. MG-01 through MG-08 have been implemented as bounded slices; creating the wider programme epic or its remaining GitHub issues still requires separate authority. Public discovery, queue offers, no-shows, AI fill, player-created standard Games, automatic private-Game successor carry-forward, and external notification transport remain deferred.
