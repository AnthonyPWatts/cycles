...
1. Complete the staged Google OIDC cutover in [#162](https://github.com/AnthonyPWatts/cycles/issues/162) after the deployed SQL lock ends. The code path, existing-Player binding transaction, canonical proxy boundary, privacy page and runbook are implemented dark; provider credentials, exact Anthony/Will emails, hosted first-login evidence and invitation removal remain operational work.
...
3. Continue shared scheduled-execution hardening through [#132](https://github.com/AnthonyPWatts/cycles/issues/132). Explicit batch-one due selection and Game-then-Cycle resolution are now in place; health, leadership, graceful shutdown, monitoring, and any later bounded several-Cycle policy remain.
