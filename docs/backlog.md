...
The current Day One guide and single-Game runtime remain implemented behaviour. The first technical prerequisites are now in place: mandatory non-zero SQL evidence; fail-closed legacy Cycle selection; a frozen online whole-state call-site allowance; additive Game/configuration/enrolment/lifecycle schema; deterministic restartable legacy backfill; v6 transfer with v1-v5 adaptation; legacy dual-write persistence; non-null Cycle/Game ownership; the exhaustive currently representable same-scope relationship matrix; normalised battle-fleet membership; and the MG-03 application/SQL boundary for redacted accounts, bounded per-player Game catalogues, neutral Game access, exact actor contexts, coherent one-Cycle views, a fixed legacy scope, and allow-listed one-Cycle commands. MG-01 and MG-02 are complete and MG-03 is underway, while MG-00's baseline player evidence remains open. Before code creates a second durable Game, online API and Worker consumers must leave the whole-state bridge, every player route must be migrated or deliberately pinned, and explicit Worker due selection, resource authorisation, and antiforgery must land. The current Worker policy remains under #132 until those bounded slices land.
