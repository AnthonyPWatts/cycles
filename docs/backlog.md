...
The current Day One guide and single-Game runtime remain implemented behaviour. The first technical prerequisites are now in place: mandatory non-zero SQL evidence; fail-closed legacy Cycle selection; a frozen online whole-state call-site allowance; additive Game/configuration/enrolment/lifecycle schema; deterministic restartable legacy backfill; v5 transfer with v4 adaptation; and legacy dual-write persistence. This completes the storage/compatibility substance of MG-01, while MG-00's baseline player evidence remains open. Before code creates a second durable Game, the programme still requires MG-02 same-scope constraints, focused player/Game/Cycle stores, every player route migrated or deliberately pinned, explicit Worker due selection, resource authorisation, and antiforgery. The current Worker policy remains under #132 until those bounded slices land.
