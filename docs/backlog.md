...
The current Day One guide and single-Game dashboard remain implemented player behaviour. MG-01 through MG-05 now provide the first technical prerequisites: the Game/configuration/enrolment/lifecycle foundation; exhaustive same-scope SQL ownership and normalised battle membership; Player-only external sign-in and focused account/admin stores; exact Game/Cycle access and command contexts; coherent scoped reads and allow-listed commands; explicit `/games/{gameId}` gameplay routes with unscoped compatibility routes pinned to the same legacy-Game handlers; resource authorisation; shared antiforgery protection for cookie-authenticated mutations; and an empty API/Worker whole-state allowance. Migration 025 and transfer v7 add immutable `Scheduled`/`SelfPaced` capability, coherent `NextTickAt`, indexed batch-one due discovery, and explicit Game-then-Cycle resolution. The integrated MG-01–05 local gate has passed; the next executable work is MG-06's roster-aware profile factory and MG-07's Games home. No current route creates or exposes a second Game.

The Games home and selected-game browser shell, second-Game creation/runtime path, roster-aware profile factory, standard/Twin Reaches catalogues, Training provisioning and tutorial journey remain unimplemented. Fresh zero-Game invitations therefore remain off until the Games home provides its intentional empty state. Worker health, leadership, graceful shutdown, monitoring and any later several-Cycle batch policy remain under [#132](https://github.com/AnthonyPWatts/cycles/issues/132). MG-00's baseline player evidence also remains open.

The dependency-labelled MG identifiers are planning labels, not live issue numbers. MG-01 through MG-05 have been implemented as bounded prerequisite slices; creating the wider programme epic or its remaining GitHub issues still requires separate authority. Public discovery, queue offers, no-shows, AI fill, player-created Games, automatic private-Game successor carry-forward, and external notification transport remain deferred.
