...
| [Q115](https://github.com/AnthonyPWatts/cycles/issues/103) | Keep `/` public and require private-alpha or Production authentication plus invited-player admission before serving `/app.html`. Keep `/health` public and continue protecting every game API independently. | The trusted playground now follows the same page boundary: its shared code protects `/app.html`, application assets, authentication routes, and game APIs while leaving the landing page, promotional media, and `/health` public. A whole-site perimeter remains an explicit option for a distinct deployment that must not be publicly discoverable. Implementation is tracked by [issue #124](https://github.com/AnthonyPWatts/cycles/issues/124). |
