...
| Orders | Durable move, hold, attack, colonise, cancellation, and supersession lifecycle with one confirmed next-tick intention per fleet, SQL Cycle-lock command mutations, deterministic game-AI/neutral/implicit Holds, sealed source/timing audit fields, and phased resolution independent of submission timestamps. | The dashboard does not expose Hold, fleet creation, or fleet splitting. Game-AI and neutral planners Hold by design in this first boundary; richer strategy and unresolved multi-faction combat remain separate work. The current implementation gives stable identifier order priority when competing Colonise intentions exceed available Population; [#139](https://github.com/AnthonyPWatts/cycles/issues/139) gates an explicit contention rule. |
...
5. Resolution proceeds in deterministic batches: resources; mandatory economy and due construction; new programme spending and construction starts; arrivals and movement; combat; colonisation; derived control, visibility, availability, and defeat state; next-window progression; factual events and Chronicle selection.
...
- Resolution runs resources; due construction; new programme spending; arrivals and movement; combat; colonisation; derived metrics; next-window progression; and one-transaction publication in that order.
