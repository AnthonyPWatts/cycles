...
The [multi-game and tutorial plan](multi-game-and-tutorial-plan.md) and its [test plan](multi-game-and-tutorial-test-plan.md) define the approved target architecture. MG-01 through MG-06 now implement its persistence, isolation, online-scope, batch-one scheduling and roster-aware profile prerequisites, but the player product remains a single legacy Game. `docs/project-state.md` remains authoritative for implemented behaviour.
...
The MG-01–06 technical hard gates have passed their integrated local evidence: scoped routes and stores, explicit Worker selection, resource authorisation, antiforgery, same-Cycle constraints, migration 025, immutable profile hashes/topology, deterministic roster materialisation, and mandatory SQL execution are verified. That clears implementation to proceed to the Games shell and private Training provisioning; it does not itself create or expose a second durable Game, and it does not authorise a new paid Worker host for the cost-capped playground.
...
The code-owned catalogue and roster-aware factory now implement the MG-06 creation boundary, including the authored Twin Reaches topology and scenario. They do not implement the Games home, a second-Game creation/runtime path, Training provisioning or the tutorial journey. Those remain MG-07 onward. A second Game must still be created only through the scoped factory and account-shell paths, never by bypassing the verified prerequisite boundary.
