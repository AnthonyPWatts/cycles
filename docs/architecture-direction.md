...
The [multi-game and tutorial plan](multi-game-and-tutorial-plan.md) and its [test plan](multi-game-and-tutorial-test-plan.md) define the approved target architecture. MG-01 through MG-09 now implement its persistence, isolation, online scope, batch-one scheduling, roster-aware profiles, account shell, private Training provisioning, and server-derived Core journey. `docs/project-state.md` remains authoritative for implemented behaviour.
...
The MG-01–09 technical hard gates have passed their integrated local evidence: scoped routes and stores, explicit Worker selection, resource authorisation, antiforgery, same-Cycle constraints, migrations 025–027, immutable profile hashes/topology, deterministic roster materialisation, the bounded account Games projection, URL-authoritative selection, private idempotent Training creation, fact-derived lesson progress and mandatory SQL execution are verified. MG-10 still owns complete responsive/accessibility states and non-visual topology, and this programme does not authorise a new paid Worker host for the cost-capped playground.
...
The persistence prerequisites include the additive foundation, its scope contract, explicit scheduling capability, and durable tutorial attempts. `GameState` and SQL carry `Game`, `CycleConfiguration`, `GameEnrolment`, `GameLifecycleEvent`, explicit participant Game scope, normalised battle-fleet membership, immutable `Scheduled`/`SelfPaced` configuration provenance and authoritative `NextTickAt`; SQL additionally owns versioned `TutorialRuns`, acknowledgements, completion/skip evidence and supersession. Migration 022 deterministically adapts the existing lineage; migration 023 enforces every currently representable same-Game or same-Cycle relationship; migration 024 hardens external identity matching; migration 025 backfills scheduling; and migrations 026–027 add restartable Training-run backfill and explicit skip evidence. State-transfer v7 validates the simulation rules and adapts v1-v6 input; tutorial attempt metadata deliberately remains outside whole-state transfer. Operational import remains pinned to the deterministic legacy Game identity even though the transfer representation can describe several Games.
...
The code-owned catalogue and roster-aware factory implement the MG-06 creation boundary, including the authored Twin Reaches topology and scenario. The MG-07 account shell enumerates existing memberships and safely switches selected-Game workspaces; MG-08 creates or resumes one private Training attempt for an allow-listed Player; MG-09 derives its four-lesson journey from selected-Cycle facts and provides active-run-gated resolution plus pause, skip, recovery and atomic fresh-attempt semantics. A second Game must still be created only through these scoped factory and account-shell paths, never by bypassing the verified boundary.
