...
The dashboard's next-test scale is the curated 8-sector, 64-system, four-empire galaxy. Each level has at most eight children, sectors and systems form irregular connected graphs, and each sector exposes exactly two gateway systems whose bridge fan-out may vary. Sector membership, gateway context, bridge routes, curated Galaxy/Sector/Local compositions, search, and the overview navigator are one client contract rather than cosmetic grouping. The current SVG map and bounded client-side lists do not promise support beyond that canonical scale or for arbitrary dense topologies; a larger target requires fresh payload, filtering, rendering, and interaction evidence.
