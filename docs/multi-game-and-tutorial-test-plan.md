...
Status: MG-01 through MG-07 prerequisite baseline implemented and verified through the integrated local technical gates. MG-03 and MG-04 cut online API and Worker consumers over to focused selected-Game stores and routes with resource authorisation, legacy-Game adapters, and antiforgery. MG-05 adds v7 scheduling state, batch-one due discovery, and explicit Game-then-Cycle resolution. MG-06 adds immutable Standard and Twin Reaches catalogues, startup hash/topology validation, deterministic roster-aware materialisation, a real first-Move simulation and SQL round trips. MG-07 adds the bounded account Games projection, server-ranked attention ledger, intentional zero-membership state, responsive account shell, URL-authoritative selection and two-tab stale-response isolation. The second-Game runtime, Training provisioning, and tutorial journey are not implemented.
