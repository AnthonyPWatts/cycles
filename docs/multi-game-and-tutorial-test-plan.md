...
Status: MG-01 through MG-06 prerequisite baseline implemented and verified through the integrated local technical gates. MG-03 and MG-04 cut online API and Worker consumers over to focused selected-Game stores and routes with resource authorisation, legacy-Game adapters, and antiforgery. MG-05 adds v7 scheduling state, batch-one due discovery, and explicit Game-then-Cycle resolution. MG-06 adds immutable Standard and Twin Reaches catalogues, startup hash/topology validation, deterministic roster-aware materialisation, a real first-Move simulation and SQL round trips. The account shell, second-Game runtime, Training provisioning, and tutorial journey are not implemented.
