...
Cloudflare overwrites `X-Cycles-Proxy-Secret` with its protected `ORIGIN_AUTH_TOKEN` before proxying dynamic requests. The API compares that credential before accepting the exact canonical host and HTTPS scheme from the Cycles-owned `X-Cycles-Canonical-Host` and `X-Cycles-Canonical-Proto` headers, then removes the proxy headers. The standard `X-Forwarded-*` headers are not used for this trust decision because hosting platforms may rewrite or combine them. A safe direct-Azure request redirects to the canonical domain, a direct mutation is refused, and `/health` remains available for Azure checks. Do not reuse the Google client secret as the origin secret.
...
The playground's Azure SQL serverless database can take roughly a minute to resume after its 60-minute idle auto-pause. Runtime store connections make up to three attempts for the observed post-login timeout (`-2`) and Azure unavailable-database response (`40613`), with a short delay between attempts. A failed one-time OIDC callback cannot be replayed; once the database is online, start a fresh sign-in from `/app.html`. Confirm a suspected cold start by matching the App Service SQL exception with a `Microsoft.Sql/servers/databases/resume` activity-log event.
