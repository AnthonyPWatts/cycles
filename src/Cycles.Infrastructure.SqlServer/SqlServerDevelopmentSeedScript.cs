...
public static class SqlServerDevelopmentSeedScript
{
    ...
    public static string Generate()
    {
        ...
        AppendInsert(script, "dbo.FleetOrders", "FleetOrderID, CycleID, FleetID, OrderType, TargetSystemID, TargetEmpireID, SubmitTick, ExecuteAfterTick, ProcessedTick, Status, RejectionReason, CreatedAt", state.FleetOrders.OrderBy(item => item.FleetOrderId), item =>
            $"({Sql(item.FleetOrderId)}, {Sql(item.CycleId)}, {Sql(item.FleetId)}, {Sql(item.OrderType)}, {Sql(item.TargetSystemId)}, {Sql(item.TargetEmpireId)}, {item.SubmitTick}, {item.ExecuteAfterTick}, {Sql(item.ProcessedTick)}, {Sql(item.Status)}, {Sql(item.RejectionReason)}, {SqlSeedTime(item.CreatedAt)})");
