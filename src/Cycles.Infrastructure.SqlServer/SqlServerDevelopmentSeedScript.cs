...
public static class SqlServerDevelopmentSeedScript
{
    ...
    public static string Generate()
    {
        ...
        AppendInsert(script, "dbo.Fleets", "FleetID, CycleID, EmpireID, FactionID, AdmiralID, FleetName, CurrentSystemID, DestinationSystemID, ArrivalTickNumber, ShipCount, Status, CreatedAt", state.Fleets.OrderBy(item => item.FleetName), item =>
            $"({Sql(item.FleetId)}, {Sql(item.CycleId)}, {Sql(item.EmpireId == Guid.Empty ? (Guid?)null : item.EmpireId)}, {Sql(item.FactionId)}, {Sql(item.AdmiralId)}, {Sql(item.FleetName)}, {Sql(item.CurrentSystemId)}, {Sql(item.DestinationSystemId)}, {Sql(item.ArrivalTickNumber)}, {item.ShipCount}, {Sql(item.Status)}, {SqlSeedTime(item.CreatedAt)})");
